...
gpu_data <- data.frame(
  ...
  Name = c(
    "Titan Xp", "Tesla V100", "Quadro RTX 8000", "A100",
    "A100", "A100", "H100", "H100", "H200"
  ),
  Value = c(12, 32, 48, 80, 80, 80, 96, 96, 141) # Memory in GiB
)
...
model_data <- data.frame(
  ...
  Name = c(
    "Transformer", "BERT-large", "GPT-2", "GPT-3",
    "OPT", "PaLM", "GPT-4", "GPT-4o", "Grok 3"
  ),
  Value = c(0.07, 0.34, 1.5, 175, 530, 540, 1000, 1800, 2700) # Parameters in billions
)

# Combine the two data frames
data <- rbind(gpu_data, model_data)
data$Category <- factor(data$Category, levels = c("Model", "GPU"))
...
p <- ggplot(
  data,
  aes(x = Year, y = Value, color = Category, shape = Category)
) +
  # Plot the points
  geom_point(size = 4) +
  # Connect the points for each category with a dashed line
  geom_line(aes(group = Category), linetype = "solid") +
  # Add non-overlapping text labels
  # geom_text_repel(aes(label = Label), size = 4, show.legend = FALSE) +
  geom_label_repel(aes(label = Label), size = 4, show.legend = FALSE) +
