...
description_for_ppl <- function(df, the_attempt) {
  ...
  if (dim(df_ppl_at1)[1] > 0) {
    ...
    at1_mem <- formatC(df_ppl_at1$load_mem_allot, format = "f", digits = 2)
    str_content <- paste0(
      abbrv1, " PPL WT: ", at1_ppl_wk, " MEM: ", at1_mem, "\n"
    )
  }
  return(str_content)
}

description_for_ppl2 <- function(df, the_attempt, the_budget = NA) {
  str_content <- ""
  if (is.na(the_budget)) {
    df_ppl_at1 <- df |> filter(attempt == the_attempt)
  } else {
    df_ppl_at1 <- df |> filter(attempt == the_attempt & bpp == the_budget)
  }
  if (dim(df_ppl_at1)[1] > 0) {
    attempt_str <- to_display_attempt(paste0(the_attempt, "-", the_budget))
    abbrv1 <- toupper(abbreviate(attempt_str))
    at1_ppl_wk <- formatC(df_ppl_at1$ppl_wikitext, format = "f", digits = 2)
    at1_mem <- formatC(df_ppl_at1$load_mem_allot, format = "f", digits = 2)
    str_content <- paste0(
    ...
    circos.track(
      ...
      panel.fun = function(x, y) {
        if (is.na(the_budget)) {
          ...
          df_mod <- df |>
            filter(module == CELL_META$sector.index & attempt == the_attempt & bit_budget == the_budget)
        }
...
plot_allot_one <- function(
    df, df_ppl, model_id, color_map, attempt1, attempt2, attempt3, budget) {
  circos.clear()
  ...
  next_budget <- boost_budget(budget)
  ...
  plot_allot_track(
    df, model_id, color_map, attempt3, "self_attn.o_proj", budget
  )
  plot_allot_track(
    df, model_id, color_map, attempt3, "self_attn.o_proj", next_budget
  )

  text(0, 0, simplify_model_id(model_id), cex = 1.5, col = "darkblue")
  descr1 <- description_for_ppl2(df_ppl, attempt1)
  descr2 <- description_for_ppl2(df_ppl, attempt2)
  descr3 <- description_for_ppl2(df_ppl, attempt3, budget)
  descr4 <- description_for_ppl2(df_ppl, attempt3, next_budget)
...
df_cfg_by_budget <- df_cfgs |>
  filter(
    bit_budget == budget | (is.na(attempt) & bit_budget == next_budget)
  ) |>
  dplyr::mutate(
...
df_ppl_disp <- df_ppl_all |>
  filter(
    (bpp == budget & (attempt == attempt1 | attempt == attempt2)) |
      ((bpp == budget | bpp == next_budget) & attempt == attempt3)
  ) |>
  select(
    c(
      "model",
